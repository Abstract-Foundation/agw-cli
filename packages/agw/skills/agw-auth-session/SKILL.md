...
- Treat `auth.init` and `auth.revoke` as explicit-intent actions. Use `--dry-run` first, then `--execute` only after user confirmation.
...
- Runtime wiring such as `AGW_HOME` and `AGW_APP_URL` belongs in env vars or top-level CLI flags, not JSON payloads.

## Default Flow

1. Inspect readiness with `agw session status --json '{"fields":["status","readiness","accountAddress","policyPreset"]}'`
2. If missing, preview bootstrap with `agw auth init --json '{"chainId":2741}' --dry-run`
3. After confirmation, execute bootstrap with `agw auth init --json '{"chainId":2741}' --execute`
4. Validate health with `agw session doctor --json '{}'`
5. Revoke only with explicit confirmation using `agw auth revoke --json '{}' --execute`
