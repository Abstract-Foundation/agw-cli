...
function resolveRiskLevel(callAllowed: boolean, valueAllowed: boolean, data: Hex, value: bigint): "low" | "medium" | "high" {
  if (!callAllowed || !valueAllowed) {
    return "high";
  }
...
function buildRiskLabels(callAllowed: boolean, valueAllowed: boolean, data: Hex, value: bigint): string[] {
  const labels: string[] = [];

  if (!callAllowed) {
    labels.push("Session policy blocks this target or selector");
  }
  if (!valueAllowed) {
    labels.push("Session policy blocks this native value amount");
  }
  if (data !== "0x") {
...
export const previewTransactionTool: ToolHandler = {
  ...
  handler: async (params, context) => {
    ...
    const callAllowed = canCallTargetWithData(session.sessionConfig, to, data);
    const valueAllowed = canTransferNativeValue(session.sessionConfig, value);
    ...
    return {
      ...
      risk: {
        level: resolveRiskLevel(callAllowed, valueAllowed, data, value),
        labels: buildRiskLabels(callAllowed, valueAllowed, data, value),
      },
      policy: {
        callAllowed,
        valueAllowed,
